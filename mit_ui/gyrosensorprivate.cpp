...
void GyroSensorPrivate::SetupI2C(){
    ...
    qInfo()<< "Open I2C bus";
    char buffer[2] = {REG_PWR_MGMT_1,0};
...
_GyroData GyroSensorPrivate::readData(){
    ...
    qDebug() << QString::number(retdata.AccelX) << QString::number(retdata.AccelY) << QString::number(retdata.AccelZ);
    qDebug() << QString::number(retdata.GyroX) << QString::number(retdata.GyroY) << QString::number(retdata.GyroZ);
